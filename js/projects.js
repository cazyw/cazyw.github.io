...
const icons = ['devicon-react-original colored',
...
const insertIcons = () => {
  const iconSection = icons.map((icon) => {
...
const insertProjects = () => {
  const projectSection = projectOverview.map((proj) => {
    let temp = projectTemplate;
    return temp.replace('PROJECT_ANCHOR', proj['anchor'])
